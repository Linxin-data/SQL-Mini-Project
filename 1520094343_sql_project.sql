...
the member name. */
SELECT tennis.court, CONCAT(tennis.firstname, ' ', tennis.surname) AS name
...
INNER JOIN Members ON Bookings.memid = Members.memid) tennis
GROUP BY tennis.court, tennis.firstname, tennis.surname
ORDER BY name
...
SELECT * FROM (SELECT Facilities.name AS facility, CONCAT ( Members.firstname, ' ', Members.surname ) AS name, 
CASE WHEN Bookings.memid = 0
THEN Facilities.guestcost * Bookings.slots
ELSE Facilities.membercost * Bookings.slots
END AS cost
...
SELECT * FROM (SELECT sub.facility, SUM(sub.cost) AS total_revenue
